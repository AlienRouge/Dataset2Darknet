...
def adjust_object_class(obj_class):
    ...
    # print(traffic_sign_classes.items())
    for classes in traffic_sign_classes.items():
        # print(classes)
        for class_ in classes[1]:
            text_class = str(class_)
            if (text_obj_class == text_class) | (
                    (re.search("_r|_n", text_class) != None) & (text_class[:-1] in text_obj_class)):
...
def add_false_data(total_false_data, total_false_negatives_dir, background_img_path, output_train_dir_path,
                   train_text_file):
...
def print_db_info(classes_counter_train, classes_counter_test):
    ...
    for i in range(0, min(len(classes_counter_train), len(classes_counter_test))):
        ...
        if total_classes == 0:
            total_classes = 1
        print('\t- CLASS ' + str(i) + " - " + classes_names[i] + ' : ' + "{:.2f}%".format(
            classes_counter_test[i] / total_classes * 100.0))
...
# Recursively clear output directory and create test/train folders.
...
def init_classes():
    global classes_names, CLASS_NUMBER

    classes_names = traffic_sign_classes.keys()
    CLASS_NUMBER = len(classes_names)
