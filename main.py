...
@click.option('--color_mode', default=COLOR_MODE,
              help='OpenCV Color mode for reading the images. (-1 (default) => color, 0 => bg).')
@click.option('--output_img_ext', default=OUTPUT_IMG_EXTENSION, help='Extension for output images. Default => .jpg')
@click.option('--verbose', default=SHOW_IMG, help='Option to show images while reading them.')
@click.option('--false_data', default=ADD_FALSE_DATA, help='Option for adding false data from datasets parsers if available.')
def main(root_path, train_pct, test_pct, color_mode, verbose, false_data, output_img_ext):
    ...
    for dataset_index in range(0, len(DATASETS)):
        ...
        DATASETS[dataset_index].update_global_variables(train_pct, test_pct, color_mode, verbose, false_data,
                                                        output_img_ext)

        # Read dataset
        classes_counter_train_partial, classes_counter_test_partial = \
            DATASETS[dataset_index].read_dataset(output_train_text_path, output_test_text_path, output_train_dir_path,
